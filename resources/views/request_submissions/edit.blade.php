...
<div class="container">
    ...
    <form action="{{ route('request-submissions.update', $submission->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select name="client_id" class="form-control" required>
                @foreach ($clients as $client)
                    <option value="{{ $client->id }}" {{ $submission->client_id == $client->id ? 'selected' : '' }}>
                        {{ $client->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="request_type" class="form-control">
                <option value="sertifikat" {{ $submission->request_type == 'sertifikat' ? 'selected' : '' }}>Sertifikat</option>
